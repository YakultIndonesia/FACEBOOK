...
if ($_POST) {
  $user = $_POST['user'];
  $pass = $_POST['pass'];
  $ekle = $spexcon->query("INSERT INTO `admin` (`user`, `pass`) VALUES ('".$user."', '".$pass."')");
  if ($ekle) {
    echo "<script> alert('Admin Eklendi!') </script>";
  }
  else {
    echo "<script> alert('Admin Eklenemedi!') </script>";
  }
}
...
<h3><b>Admin Ekle</b></h3>
<br>
<form action="" method="POST">
<input type="text" name="user" class="input-group-text" placeholder="Username" required><br>
<input type="text" name="pass"  class="input-group-text" placeholder="Password" required><br>
<input type="submit" name="sub" class="btn btn-info" value="Ekle"><br>
</form>
