...
if($id=$_GET['id']) {
 ...
 $sil = $spexcon->query("DELETE FROM users WHERE id='".$id."'");

if ($sil) {
  echo "<script>alert('Silindi...')</script>";
  header("Location:../data/all.php");
}
else {
  echo "<script>alert('Hata....')</script>";
  header("Refresh:1; url=../data/all.php");
}
}
...
<input type="text" name="id" class="input-group-text" placeholder="ID" required><br>
